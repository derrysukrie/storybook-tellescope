import { type AvatarProps, Badge, Avatar as MuiAvatar, styled } from '@mui/material'
...
interface CustomAvatarProps extends Omit<AvatarProps, 'color' | 'badge' | 'size'> {
    color?: AvatarColor
    size?: AvatarSize
    badge?: boolean
}
...
const CustomAvatar: React.FC<CustomAvatarProps> = ({ sx, color, size, ...props }) => {
    return (
        <MuiAvatar
            {...props}
            sx={{
...
const Avatar: React.FC<CustomAvatarProps> = ({ sx, color, size, badge, ...props }) => {
    return badge ? (
        <StyledBadge
            overlap="circular"
