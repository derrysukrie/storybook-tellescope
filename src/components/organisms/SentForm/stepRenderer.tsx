    ...
    lazy(() =>
        import(`./Steps/${path}.tsx`)
            ...
            .catch(error => {
                ...
                return import(`./Steps/StepSkeleton`).then(module => ({
                    default: module.StepSkeleton,
                }))
            })
    )
...
const DateComponent = lazyLoad('Date', 'Date')
...
const withSuspense = <T extends Record<string, unknown>>(
...
const renderTextStep = (
    Component: React.ComponentType<{ title?: string; helperText?: string }>,
    step: StepConfig
) =>
    withSuspense(Component, {
        title: 'title' in step ? step.title : undefined,
        helperText: 'helperText' in step ? step.helperText : undefined,
    })

// Helper function for steps with title and placeholder
const renderDateStep = (
    Component: React.ComponentType<{ title?: string; placeholder?: string }>,
    step: StepConfig
) =>
    withSuspense(Component, {
        title: 'title' in step ? step.title : undefined,
...
export const renderStep = (step: StepConfig) => {
    ...
    if (type === 'date') return renderDateStep(DateComponent, step)
