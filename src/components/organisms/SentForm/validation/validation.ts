import type { FormData, QuestionsGroupStepConfig, StepConfig } from '../types/types'
...
const validateAddress = (stepData: AddressData): boolean => {
    return (
        hasValue(stepData?.addressLine1) &&
...
export const isStepValid = (step: StepConfig, formData: FormData, checked: boolean): boolean => {
    ...
    // Always valid steps
    if (
        validationRules.alwaysValid.includes(type as (typeof validationRules.alwaysValid)[number])
    ) {
        return true
    }
    ...
    if (
        validationRules.requiresCheckbox.includes(
            type as (typeof validationRules.requiresCheckbox)[number]
        )
    ) {
        return checked
    }
    ...
    if (
        validationRules.requiresString.includes(
            type as (typeof validationRules.requiresString)[number]
        )
    ) {
    ...
    if (
        validationRules.requiresArray.includes(
            type as (typeof validationRules.requiresArray)[number]
        )
    ) {
    ...
    if (
        validationRules.requiresNumber.includes(
            type as (typeof validationRules.requiresNumber)[number]
        )
    ) {
