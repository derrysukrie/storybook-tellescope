...
function ProviderWrapper(props: StateProps & { children?: ReactNode }) {
    ...
    const [internalFilter, internalSetFilter] = useState<{
        value: string
        filterOption: string
    } | null>(
    ...
    const [internalSort, internalSetSort] = useState<SortItem[]>(
        'value' in props && props.appearance === 'sort' && props.value
    ...
    const value = {
        ...
        onCloseFilter:
            'onClose' in props && props.appearance === 'filter' ? props.onClose : undefined,
    }

    return (
        <DetailSelectContext.Provider value={value}>{props.children}</DetailSelectContext.Provider>
    )
}
...
const DetailSelectContent = ({ appearance }: { appearance: 'sort' | 'filter' }) => {
    return appearance === 'filter' ? <DetailSelectPending /> : <DetailSelectSort />
}
...
function DetailSelectSort() {
    ...
    const handleClickOnAddSort = (e: React.MouseEvent<HTMLLIElement>) =>
        setAnchorAddSort(e.currentTarget)
    ...
    const handleSetSortField = (field: string, replaced: string) => {
        ...
        onChangeSort?.(
            field,
            (sort.find(item => item.field === replaced)?.order as 'ascending' | 'descending') ||
                'ascending',
            replaced
        )
    }
...
function SortRow({
    ...
    const { availableSortFields } = useDetailSelectContext()
    ...
    const search = availableSortFields.filter(
        f =>
    ...
    return (
        <Box sx={{ display: 'flex', gap: '2px' }}>
            ...
            >
                ...
                {/* </ListSubheader> */}
                {search.map(f =>
                    usedFields.includes(f.value) ? null : (
                        // Skip if the field is already used
                        <MenuItem
                            key={f.value}
                            value={f.value}
                            disabled={usedFields.includes(f.value) && f.value !== field}
                        >
            ...
            >
                ...
                {search.map(f =>
                    usedFields.includes(f.value) ? null : (
                        ...
                        <MenuItem
                            key={f.value}
                            onClick={() => onAddSort(f.value)}
                            disabled={usedFields.includes(f.value)}
                        >
                            <ListItemIcon>{f.icon}</ListItemIcon>
                            <ListItemText>{f.label}</ListItemText>
                        </MenuItem>
...
const DetailSelectPending = () => {
    ...
    const isEmptyOrNotEmpty =
        filter?.filterOption === 'is empty' || filter?.filterOption === 'is not empty'
    ...
    return (
        <>
            <Button
                ...
                sx={{
                    backgroundColor:
                        filter?.value || isEmptyOrNotEmpty ? '#0288D114' : 'transparent',
                    borderRadius: '16px',
                    display: 'inline-flex',
                    alignItems: 'center',
                    px: 1.5,
                    ...
                    '&:hover': {
                        backgroundColor:
                            filter?.value || isEmptyOrNotEmpty ? '#0288D129' : '#EEEDF4',
                    },
                    boxShadow: 'none !important',
                }}
                onClick={handleClick}
            >
                ...
                >
                    <Typography
                        sx={{
                            color: filter?.value || isEmptyOrNotEmpty ? '#0288D1' : '#0000008F',
                            fontWeight: 500,
                            ml: 0.5,
                        }}
                        variant="body2"
                    >
                        ...
                        >
                            ...
                            >
                                {
                                    // filter?.value || "Value"
                                    filter?.filterOption && isEmptyOrNotEmpty
                                        ? `${filter?.filterOption}`
                    ...
                    <KeyboardArrowDownIcon
                        sx={{ color: !filter?.value ? '#0000001F' : '#0288D1', ml: 0.5 }}
                    />
                </Badge>
            </Button>
...
function FilterRow({
    ...
    const { filterOptions } = useDetailSelectContext()
...
function useDetailSelectContext() {
    const context = useContext(DetailSelectContext)
    if (!context) {
        throw new Error('useDetailSelectContext must be used within DetailSelectContext.Provider')
    }
    return context
}
