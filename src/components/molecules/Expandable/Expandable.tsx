...
export const DisplayFields: React.FC<{
    ...
    onTogglePin: () => void
    onToggleHidden: () => void
}> = ({ state = 'view', label, value, isPinned, hidden, onTogglePin, onToggleHidden }) => {
    return (
        <Box display="flex" alignItems="center" justifyContent="space-between">
            ...
            <Box display="flex" alignItems="center">
                {state === 'edit' && (
                    <IconButton onClick={onToggleHidden} size="small">
                        {!hidden ? (
                            ...
                            <VisibilityOffOutlined sx={{ fontSize: 18, color: 'rgba(0, 0, 0)' }} />
                        )}
                    </IconButton>
                )}
                {state === 'edit' && (
                ...
                <IconButton size="small">
                    <EditOutlined fontSize="small" sx={{ fontSize: 18, color: 'rgba(0, 0, 0)' }} />
                </IconButton>
                <IconButton size="small">
                    <DeleteOutline fontSize="small" sx={{ fontSize: 18, color: 'rgba(0, 0, 0)' }} />
                </IconButton>
            </Box>
        </Box>
    )
